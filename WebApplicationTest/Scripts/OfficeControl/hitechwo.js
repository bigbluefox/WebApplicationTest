//以下是程序引用页面的定义
var editMScode = "editoffice.aspx"; //修改MS OFFICE文档页面程序
var editWPScode = "editwps.aspx"; //修改WPS文档页面程序
var savecode = "../../WebOffice/DocSave.aspx"; //保存文档程序程序
...
function editoffice(url, newofficetype) {
    ...
    //根据文档Url和newofficetype修改文档,如果有url是修改已有文档,如果为空根据newofficetype新建文档
...
function getParagraphs() {
    var resultText = ""; //返回获取到的大纲集合字符串
    try {
        var paras = TANGER_OCX_OBJ.ActiveDocument.Paragraphs; //Paragraphs 集合包含了所选内容、范围或文档中的所有段落
        var termsNum = 0;
        var json = "[";
        var temp;
        for (var i = 1; i <= paras.Count; i++) //遍历当前文档所有段落
        {

            if (paras(i).Format.OutlineLevel < 10) {
                temp = termsNum.toString().split('.');
                termsNum = "";
                for (var level = 0; level < paras(i).Format.OutlineLevel - 1; level++) {

                    termsNum += temp[level] + ".";
                }
                if (temp.length < paras(i).Format.OutlineLevel) {
                    termsNum += "1";
                }
                else {
                    termsNum += Number(temp[paras(i).Format.OutlineLevel - 1] == null ? 0 : temp[paras(i).Format.OutlineLevel - 1]) + 1;
                }
                resultText += termsNum + ' ' + paras(i).Range.Text + "<br>"; //获取大纲文本内容
                json += ',{"Level":"' + paras(i).Format.OutlineLevel + '","Title":"' + termsNum + '","Content":"' + paras(i).Range.Text.toString() + '"}';
            }
...
function TANGER_OCX_OpenLocalDoc(fileUrl) {
    intializePage();
    if (fileUrl != null || fileUrl != "") {
        TANGER_OCX_OBJ.OpenLocalFile(fileUrl, false);
    }
}
...
function saveFileAsPdfToUrl() {
    ...
    if (IsFileOpened && TANGER_OCX_OBJ.IsPDFCreatorInstalled()) {
        ...
        alert("不能执行保存,没有修改文档或者没有安装PDF虚拟打印机!");
    }
}
...
function setFileOpen(boolvalue) {
    TANGER_OCX_OBJ.FileOpen = boolvalue; //是否允许打开
}
function setIsNoCopy(boolvalue) {
    TANGER_OCX_OBJ.IsNoCopy = boolvalue; //是否禁止粘贴
}

//function IsSetReadOnly(IsReadOnly, PassWord) {
...
//自定义控件
function CustomControlStyle() {
    var myobj = TANGER_OCX_OBJ;
    myobj.AddCustomMenu2(1, "条款");
    myobj.AddCustomMenuItem2(1, 0, -1, false, "条款", false);
}

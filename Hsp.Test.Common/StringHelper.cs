...
namespace Hsp.Test.Common
{
    ...
    /// 2、GetStrArray(string str) 把字符串按照[,]分割 换为数据  
    ...
    /// 10、GetSubStringList(string oStr, char sepeater)把字符串按照指定分隔符装成 List 去除重复  
    ...
    public class StringHelper
    {
        ...
        public static string SqlSafeString(string s, bool isDel)
        {
            if (isDel)
            {
                s = s.Replace("'", "");

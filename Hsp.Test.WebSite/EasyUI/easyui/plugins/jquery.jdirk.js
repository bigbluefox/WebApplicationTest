...
(function (window, $, undefined) {
    ...
    coreMath.fn = coreMath.prototype = {};
    ...
    coreMath.add = function (a, b) {
        if (!coreUtil.isNumeric(a) || !coreUtil.isNumeric(b)) { throw "传入的参数 num 必须是一个数值"; }
        var c, d, e;
        var aStr = (a || 0).toString(), bStr = (b || 0).toString();
        if (aStr.indexOf(".") > -1) {
            c = aStr.split(".")[1].length;
        } else {
            c = 0;
        }
        if (bStr.indexOf(".") > -1) {
            d = bStr.split(".")[1].length;
        } else {
            d = 0;
        }
        return e = Math.pow(10, Math.max(c, d)), (coreMath.mul(a, e) + coreMath.mul(b, e)) / e;
    };

    coreMath.sub = function (a, b) {
        if (!coreUtil.isNumeric(a) || !coreUtil.isNumeric(b)) { throw "传入的参数 num 必须是一个数值"; }
        var c, d, e;
        var aStr = (a || 0).toString(), bStr = (b || 0).toString();
        if (aStr.indexOf(".") > -1) {
    ...
    coreMath.div = function (a, b) {
        if (!coreUtil.isNumeric(a) || !coreUtil.isNumeric(b)) { throw "传入的参数 num 必须是一个数值"; }
        var c, d, e = 0,
        f = 0;
        var aStr = (a || 0).toString(), bStr = (b || 0).toString();
        if (aStr.indexOf(".") > -1) {
            e = aStr.split(".")[1].length;
        }
        if (bStr.indexOf(".") > -1) {
            f = bStr.split(".")[1].length;
        }
        return c = Number(aStr.replace(".", "")), d = Number(bStr.replace(".", "")), coreMath.mul(c / d, Math.pow(10, f - e));
    };

    //  将指定的数值进行分割，以实现诸如千分位的效果；该函数定义如下参数：
    //      num：    要分割的数值，支持 number 类型和 string 类型；
    //      len：    分割的间隔长度，该参数可选，默认为 3；
    //      separator：  分割符号，该参数可选，默认为 ,（英文逗号）；
    //  返回值：返回分割后的数值字符串。
    coreMath.separate = function (num, len, separator) {
        if (!coreString.isNumeric(num)) { throw "传入的参数 num 必须是一个数值"; }
        len = len || 3;
        if (!coreUtil.isNumeric(len)) { throw "传入的参数 len 必须是一个数值"; }
        if (len < 1) { throw "传入的参数 num 必须是一个大于 0 的整数"; }
        var num2 = coreUtil.isNumeric(num) ? num : coreString.toNumeric(num);
        var numStrTemp = num2.toString(), numStr1, numStr2;
        var isFloat = coreString.contains(numStrTemp, ".");
        if (isFloat) {
            var temp = numStrTemp.split(".");
            numStr1 = temp[0];
            numStr2 = temp[1];
        } else {
            numStr1 = numStrTemp;
        ...
        while (numStr1.length > len) {
            result = separator + numStr1.slice(-len) + result;
            numStr1 = numStr1.slice(0, numStr1.length - len);
        }
        if (numStr1) { result = numStr1 + result; }
        if (numStr2) { result = result + "." + numStr2; }

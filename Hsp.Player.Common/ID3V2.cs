...
namespace Hsp.Player.Common
{
    ...
    internal class ID3V2
    {
        ...
        public ID3V2(IntPtr a0)
        {
            if (Utils.IntPtrAsStringAnsi(a0, 3) == "ID3")
            {
                this.index += 3;
                this.byte1 = Marshal.ReadByte(a0, this.index);
                this.index++;
                this.byte2 = Marshal.ReadByte(a0, this.index);
                this.index++;
                this.byte3 = Marshal.ReadByte(a0, this.index);
                this.index++;
                int num = this.c(a0, this.index);
                this.index += 4;
                bool flag = (this.byte3 & 0x40) > 0;
                int num2 = 0;
                if (flag)
                {
                    num2 = this.c(a0, this.index);
                }
                this.data = new byte[num + 10];
                Marshal.Copy(a0, this.data, 0, num + 10);
        ...
        private byte[] a(byte[] a0)
        {
            List<byte> list = new List<byte>(a0);
        ...
        private object a(int A_0, short a1)
        {
        ...
        private string a(byte[] a0, ref int a1)
        {
            StringBuilder builder = new StringBuilder();
            try
            {
                char ch;
                while ((ch = (char)a0[a1]) != '\0')
                {
                    builder.Append(ch);
                    a1++;
                }
        ...
        private int a(IntPtr a0, int a1)
        {
            byte[] buffer = new byte[] { Marshal.ReadByte(a0, a1), Marshal.ReadByte(a0, a1 + 1), Marshal.ReadByte(a0, a1 + 2) };
            return (((buffer[0] << 0x10) | (buffer[1] << 8)) | buffer[2]);
        }

        private string a(byte[] a0, ref int a1, Encoding a2)
        {
            ...
            try
            {
                if (a0[0] == 1)
                {
                    if ((a0[a1] != 0xfe) || (a0[a1 + 1] != 0xff))
                    {
                        if ((a0[a1] != 0xff) || (a0[a1 + 1] != 0xfe))
                        {
                            return str;
                        }
                        a2 = Encoding.Unicode;
                ...
                str = a2.GetString(a0, a1, index - a1);
                a1 = index;
                if (num == 2)
                {
                    a1++;
        ...
        public TagPicture a(byte[] a0, short a1, int a2, bool a3)
        {
            if (a0 == null)
            {
                return null;
            ...
            try
            {
                ...
                if ((a1 & 1) != 0)
                {
                    this.a(a0[0], a0[1], a0[2], a0[3]);
                    index = 4;
                }
                Encoding encoding = this.a(a0[index]);
                index++;
                if (a3)
                {
                    str = "Unknown";
                    if (((a0[index] == 0x4a) && (a0[index + 1] == 80)) && (a0[index + 2] == 0x47))
                    {
                        str = "image/jpeg";
                    }
                    else if (((a0[index] == 0x47) && (a0[index + 1] == 0x49)) && (a0[index + 2] == 70))
                    {
                        str = "image/gif";
                    }
                    else if (((a0[index] == 0x42) && (a0[index + 1] == 80)) && (a0[index + 2] == 0x4d))
                    {
                        str = "image/bmp";
                    }
                    else if (((a0[index] == 80) && (a0[index + 1] == 0x4e)) && (a0[index + 2] == 0x47))
                    {
                        str = "image/png";
                    }
                    index += 3;
                ...
                index++;
                string description = this.a(a0, ref index, encoding);
                index++;
                int length = a0.Length - index;
                if (length <= 0)
                {
                    return picture;
                }
                destinationArray = new byte[length];
                Array.Copy(a0, index, destinationArray, 0, length);
                if ((a1 & 2) != 0)
                {
                ...
                picture = new TagPicture(a2, str, unknown, description, destinationArray);
            }
            catch
            {
            }
        ...
        private int a(byte a0, byte a1, byte a2, byte a3)
        {
            if ((((a0 & 0x80) != 0) || ((a1 & 0x80) != 0)) || (((a2 & 0x80) != 0) || ((a3 & 0x80) != 0)))
            {
                throw new FormatException("Found invalid syncsafe integer");
            }
            return ((((a0 << 0x15) | (a1 << 14)) | (a2 << 7)) | a3);
        }
        ...
        private int b(IntPtr a0, int a1)
        {
            byte[] buffer = new byte[] { Marshal.ReadByte(a0, a1), Marshal.ReadByte(a0, a1 + 1), Marshal.ReadByte(a0, a1 + 2), Marshal.ReadByte(a0, a1 + 3) };
        ...
        private int c(IntPtr a0, int a1)
        {
            byte[] buffer = new byte[] { Marshal.ReadByte(a0, a1), Marshal.ReadByte(a0, a1 + 1), Marshal.ReadByte(a0, a1 + 2), Marshal.ReadByte(a0, a1 + 3) };

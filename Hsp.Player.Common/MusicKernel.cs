...
namespace Hsp.Player.Common
{
    ...
    public class MusicKernel : IDisposable
    {
        ...
        /*
        Bass类库分享出来，不是很完善可能有一些Bug，你们可以参考一下
        ...
        */
        ...
        public double Schedule
        {
            get
            {
                ...
                {
                    schedule = Bass.BASS_ChannelBytes2Seconds(stream, Bass.BASS_ChannelGetPosition(stream))/
                               Bass.BASS_ChannelBytes2Seconds(stream, Bass.BASS_ChannelGetLength(stream));
                }
                return schedule;
            }
            set
        ...
        /// DSP插件集合
        /// </summary>
        public List<DSP> DSPs = new List<DSP>();
        ...
        internal bool CreateStream()
        {
            if (stream != 0)
            {
                if (Bass.BASS_ChannelIsActive(stream) == BASSActive.BASS_ACTIVE_PLAYING ||
                    Bass.BASS_ChannelIsActive(stream) == BASSActive.BASS_ACTIVE_PAUSED)
                {
                    ...
                    if ((bool) restart)
                    {
                        ...
                        if (DSPs.Count > 0)
                        {
                            for (int i = 0; i < DSPs.Count; i++)
                            {
                                if (DSPs[i].IsActive)
                                {
                                    DSPs[i].SetDSPChannel(stream);
                                    DSPs[i].SetMusicFileName(fileName);
                                    DSPs[i].SetMusicTitle(musicTags.Title);
                                }
                            }
                        }
                    }
